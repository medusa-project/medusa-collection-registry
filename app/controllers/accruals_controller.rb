class AccrualsController < ApplicationController
  ...
  def submit
    ...
    if Accrual.new(cfs_directory: cfs_directory).directory_accruable?
      ...
      flash[:notice] = "This directory already has an accrual pending. Please wait for that accrual to finish before beginning another."
      redirect_to cfs_directory
    end
  end
