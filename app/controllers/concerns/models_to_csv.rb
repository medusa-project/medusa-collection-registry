...
module ModelsToCsv
  ...
  def items_to_csv(items, csv_options = {})
    models_to_csv(items, {requester_info: 'Requester Info', ebook_status: 'E-Book Status',
                          external_link: 'External Link', reviewed_by: 'Reviewed By', barcode: 'Barcode',
                          ...
                          unique_identifier: 'Unique Identifier', source_media: 'Source Media',
  ...
  def file_stats_to_csv(content_type_hashes, file_extension_hashes)
    CSV.generate do |csv|
      csv << ['Digital Content Format', 'Number of Files', 'Number Tested', 'Percentage Tested', 'Size']
