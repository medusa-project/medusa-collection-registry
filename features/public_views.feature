Feature: Public views
  ...
  Background:
    ...
    And the collection with title 'Dogs' has child file groups with fields:
      | title | type              | description     | staged_file_location |
    ...
    And the file group titled 'DogFiles' has cfs root 'dogs' and delayed jobs are run
    And the file group titled 'CatFiles' has cfs root 'cats' and delayed jobs are run
    And the collection with title 'Dogs' has public rights
    And the file group with title 'DogFiles' has public rights
    And the collection with title 'Cats' has private rights
    And the file group with title 'CatFiles' has private rights
  ...
  Scenario: View allowed file group metadata
    When I public view the file group with title 'DogFiles'
    Then I should see all of:
      | DogFiles | dog summary |
    And I should not see '/private/location'

  Scenario: Attempt to view restricted file group metadata
    When I public view the file group with title 'CatFiles'
    Then I should be redirected to the unauthorized page

  Scenario: View allowed cfs directory metadata
    When I public view the cfs directory for the file group titled 'DogFiles' for the path '.'
    Then I should see all of:
      | intro.txt |

  Scenario: Attempt to view restricted cfs directory metadata
    When I public view the cfs directory for the file group titled 'CatFiles' for the path '.'
    Then I should be redirected to the unauthorized page

  Scenario: View allowed cfs file metadata
    When I public view the cfs file for the file group titled 'DogFiles' for the path 'intro.txt'
    Then I should see all of:
      | intro.txt | 9 Bytes | f346bd77c403097b0656ecd011e8c118 | text/plain |

  Scenario: Attempt to view restricted cfs file metadata
    When I public view the cfs file for the file group titled 'CatFiles' for the path 'intro.txt'
    Then I should be redirected to the unauthorized page
  ...
  Scenario: Attempt to download restricted file
    When I public view the cfs file for the file group titled 'DogFiles' for the path 'intro.txt'
    And the file group with title 'DogFiles' has private rights
    And I click on 'Download'
    Then I should be redirected to the unauthorized page
  ...
  Scenario: Go from directory to file
    ...
    Then I should be public viewing the cfs file for the file group titled 'DogFiles' for the path 'intro.txt'

  Scenario: Go from directory to file group
    When I public view the cfs directory for the file group titled 'DogFiles' for the path '.'
    And I click on 'DogFiles'
    Then I should be on the public view page for the bit level file group with title 'DogFiles'
