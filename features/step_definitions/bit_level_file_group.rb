...
Then(/^the file group named '(.*)' should not have an assessment scheduled$/) do |name|
  expect(Job::CfsInitialFileGroupAssessment.find_by(file_group_id: BitLevelFileGroup.where(name: name).first.try(:id))).to be_nil
end
