...
namespace :medusa do
  ...
  task schedule_amqp_amazon_backups: :environment do
    begin
      ...
      file_groups = BitLevelFileGroup.where(id: file_group_ids)
      file_groups.each do |file_group|
        begin
          unless AmazonBackup.find_by(cfs_directory_id: file_group.cfs_directory_id, date: Date.today)
            AmazonBackup.create_and_schedule(User.find_by_uid('hding2@illinois.edu'), file_group.cfs_directory)
            puts "Scheduled AMQP backup for file group #{file_group.id}."
          end
        rescue Exception => e
          GenericErrorMailer.error("Unable to create Amazon Backup for AMQP accrual group: #{file_group.id}. Details: #{e}",
